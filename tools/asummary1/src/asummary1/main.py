...
def run():
    """Main entry point. Default: GUI. Use --cli for terminal mode."""
    # CLI mode — only with explicit flag
    if "--cli" in sys.argv:
        pass  # continue to CLI path below
    else:
        # Default: launch GUI window
        from asummary1.gui_viewer import show_summary_window
        show_summary_window()
        return

    print("🔍 ASummary1 — reading emails processed by AMail...\n")
